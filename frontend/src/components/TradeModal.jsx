...
export default function TradeModal({ stock, onClose, onSuccess }) {
  ...
  const fetchUserData = async () => {
    try {
      ...
      setBalance(bal);
      const h = holdings.find((x) => x.symbol === stock.symbol);
      setHolding(h || null);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (mode === "buy") {
        ...
        setMessage({ type: "success", text: "매수 완료!" });
      } else {
        await sellStock(payload);
        setMessage({ type: "success", text: "매도 완료!" });
      }
      await fetchUserData();
      onSuccess?.();
      setTimeout(() => onClose(), 1200);
    } catch (e) {
      const msg = e.response?.data || "주문 실패";
      setMessage({ type: "error", text: typeof msg === "string" ? msg : "주문 실패" });
      ...
      <div className="trade-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="trade-modal-body">
          ...
          <div className="tm-total">
            ...
            <strong>{fmt(total)}{isDomestic(stock.market) ? "원" : "$"}</strong>
          </div>
